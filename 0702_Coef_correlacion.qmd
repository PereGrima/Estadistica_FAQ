# 7.2 ¿De dónde sale la fórmula del coeficiente de correlación? {.unnumbered #correlacion}

Simplemente se divide la expresión de la covarianza por el producto de las desviaciones típicas de las dos variables $X$ e $Y$. Es fácil comprobar que desaparecen los denominadores tanto de la covarianza como de las desviaciones típicas, quedando: $$ r  = \frac{\sum_{i=1}^n \left ( x_i - \bar{x} \right ) \left ( y_i - \bar{y} \right ) } 
...
Si la correlación es perfecta todos los puntos estarán alineados según una recta, es decir: $y_i =a+bx_i$. Además, seguro que[^0702_coef_correlacion-1]: $\bar {y} =a+b \bar{x}$. Así pues, podemos escribir el numerador de la expresión de $r$ de la forma:

[^0702_coef_correlacion-1]: <small>Si todos los puntos están sobre la recta: $\sum y_i = \sum (a+bx_i) \rightarrow n\bar{y} = na + bn \bar{x}$</small>
